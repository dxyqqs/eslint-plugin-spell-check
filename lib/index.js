  ...
  rules: {
    ...
    "wording": {
      ...
      create: function (context) {
        return {
          Program() {
            console.warn(context.getSourceCode().text.replace('_=', ''))
            const config = csl.mergeSettings(csl.getDefaultSettings(), csl.getGlobalSettings());
            const {
              data,
              error
            } = syncCheckText(context.getSourceCode().text.replace('_=', ''), config)
            if (error) return
